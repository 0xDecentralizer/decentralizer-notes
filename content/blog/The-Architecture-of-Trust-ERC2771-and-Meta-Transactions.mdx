---
title: "The Architecture of Trust: ERC2771 and Meta-Transactions"
date: "2025-02-12"
...
## Part I: Context — The Template You Didn't Know You Needed
...
## Part II: The Calldata Trick — How ERC2771 Actually Works
...
[Alice's address: 20 bytes] ← Hidden here!
```
...
## Part III: The Forwarder — The Trust Anchor
...
## Part IV: Security — The Devil in the Details
...
## Part V: Performance — The Overhead Analysis
...
## Part VI: Design Decisions — The Trade-offs
...
**ERC2771**:
```text
User → Signs → Relayer → Forwarder → Target Contract
              (Trusted)
```

**EIP-4337**:
```text
User → Signs → Bundler → EntryPoint → UserOperation → Target Contract
...
1. **Context isn't useless** — it's a template for future extensibility
2. **Assembly isn't premature optimization** — when it saves 700x in gas, it matters
3. **Immutability is a feature** — constraining yourself prevents attacks
4. **Trust models are explicit** — ERC2771 admits its centralization
5. **Trade-offs are documented** — no solution is perfect for all use cases
